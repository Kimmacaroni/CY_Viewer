...
6. `docs/WORK_LOG.md`에 시작 시각, 담당 주체, 브랜치/worktree, 기준 SHA, 범위, 검증 계획을 기록한다.
...
9. 구조·버전·절차·위험이 바뀌면 본 문서를 갱신하고, 작업 로그 완료 항목에 결과와 미검증 사항을 기록한다.
10. `git status`, `git diff`, `git diff --check`로 변경 범위와 민감정보/산출물 포함 여부를 확인한 뒤 커밋한다.
11. 최종 커밋 SHA와 검증 근거를 인계한다. push·PR·릴리스·배포는 사용자의 별도 명시 지시가 있을 때만 수행한다.
